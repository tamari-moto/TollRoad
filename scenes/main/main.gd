...
##
## 素の Node であって Node2D ではない。画面は UI（CanvasLayer）から下の
## Control と、大陸図の SubViewport が持つ 3D で構成されており、2D の座標・
## 変換・描画順はどこにも要らない。Node2D にしておくと使わない変換を毎フレーム
## 抱えるうえ、「ここは 2D ゲームである」という誤った読み方を誘う。
##
...
var _logistics_debug_window: LogisticsDebugWindow
...
func _configure() -> void:
	...
	if _logistics_debug_window == null:
		...
		_panels.append(_logistics_debug_window)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action("tr_logistics_debug_toggle"):
		_logistics_debug_window.toggle_visible()
		get_viewport().set_input_as_handled()
		return
