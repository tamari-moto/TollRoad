...
M7 まで完了（[roadmap.md](docs/roadmap.md) 参照）。**仕様書の全画面が揃い、
マウスだけで全機能が操作できる**。市場・積荷・大陸図・製作所・相場メモ・
島と装備・航海日誌・休息。解像度は 1280x720。

検証シナリオは `scripts/systems/scenario_m1.gd` 〜 `scenario_m7.gd` の7本。
