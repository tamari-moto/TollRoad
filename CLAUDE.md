...
M5 まで完了（[roadmap.md](docs/roadmap.md) 参照）。ゲームロジックは60日通しで
動き、HUD と休息ボタンがマウスで操作できる。解像度は 1280x720。

検証シナリオは `scripts/systems/scenario_m1.gd` 〜 `scenario_m5.gd` の5本。
新しい機能を足したら対応するシナリオも足すこと。

未着手: 市場・積荷・大陸図の画面（M6）、製作所・相場メモ・島・航海日誌の画面（M7）、
リザルトと調整（M8）。
