...
**都市ごとの在庫と需要を実装済み**（2026-08-13）。市場は無限ではなく、都市×品目ごとに
...
掛け率だけを返す。**価格を作る場所は `price_table.gd` の1箇所に閉じること**（両方が
...
都市ペアごとの実質コスト・日数・襲撃リスクが環のときと変わる。上記の5→9拡張の
再検証とまとめて、実機での通しプレイで測り直すこと。

**黒ゾーンでレイヴンスパイアと直結するゲートをアイアンホロウ・フォックスミア・
シルバーミアの3都市に絞った際も、同じ理由で上表は未再検証。** ゲートでない
都市からレイヴンスパイアへ向かう実質コスト・日数が増える（王道でゲートまで
歩く分）。このゲート選択は3都市とも王道で中心寄りのため、黒ゾーンを2区間
経由する方が王道の迂回より安くなる都市ペアは存在しない（手計算で確認済み。
王道の直径である oakhaven↔wyndham 間ですら、王道の大迂回1250の方が
黒ゾーン2区間ルート1550より安い）。レイヴンスパイアは中継地点としてではなく、
純粋な目的地として使う設計になった。

**在庫と需要を入れた際も、上表は未再検証。** ただし意図された成長ルートが
残っていることは確認してある（20シードの自動プレイで純資産の中央値 621,261 =
MASTER TRADER。従来の実測 857,345 と同じ帯）。一方**資源をそのまま売るだけの
戦略は需要の上限で頭打ちになり、大きく落ちる**（中央値 5,942 = BANKRUPT）。
これは製作を挟ませる意図どおりだが、上表の「資源を安く買って高く売るだけ =
57,763 / SURVIVOR」はもう当てはまらない。実機での通しプレイで測り直すこと。
