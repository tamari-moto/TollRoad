...
M2 まで完了（[roadmap.md](docs/roadmap.md) 参照）。品目9種・都市6つ・価格計算・
売買・移動・日送り・相場メモ・製作・黒ゾーンの襲撃がヘッドレスで動く。

検証シナリオは `scenario_m1.gd`（交易）と `scenario_m2.gd`（製作・襲撃）の2本。
新しい機能を足したら対応するシナリオも足すこと。

未着手: 島と騎乗（M3）、60日通しとランク判定（M4）、UI 一式（M5〜）。
