...
func _test_value_types() -> void:
	...
	_check(typeof(restored.prices.get_price(GameData.INITIAL_CITY, "ore")) == TYPE_INT,
		"価格が int で戻る",
		type_string(typeof(restored.prices.prices[GameData.INITIAL_CITY]["ore"])))
...
func _test_rng_continuity() -> void:
	...
	for i: int in 10:
		original.rest()
		series.append("%d|%d|%d" % [
			original.prices.get_price(GameData.CAERLEON, "sword"),
			original.prices.get_price(GameData.INITIAL_CITY, "ore"),
	...
	for i: int in 10:
		...
		restored_series.append("%d|%d|%d" % [
			restored.prices.get_price(GameData.CAERLEON, "sword"),
			restored.prices.get_price(GameData.INITIAL_CITY, "ore"),
	...
	for i: int in 10:
		...
		broken_series.append("%d|%d|%d" % [
			broken_session.prices.get_price(GameData.CAERLEON, "sword"),
			broken_session.prices.get_price(GameData.INITIAL_CITY, "ore"),
	...
	for i: int in 10:
		...
		json_series.append("%d|%d|%d" % [
			json_session.prices.get_price(GameData.CAERLEON, "sword"),
			json_session.prices.get_price(GameData.INITIAL_CITY, "ore"),
...
func _test_unknown_ids() -> void:
	...
	partial["prices"].erase(GameData.CAERLEON)
...
func _test_file_io() -> void:
	...
	if loaded != null:
		...
		for i: int in 8:
			original.rest()
			a.append(str(original.prices.get_price(GameData.CAERLEON, "sword")))
			loaded.rest()
			b.append(str(loaded.prices.get_price(GameData.CAERLEON, "sword")))
...
func _test_version_mismatch() -> void:
	...
	data["current_city"] = GameData.INITIAL_CITY
...
func _test_save_lifetime() -> void:
	...
	broken["memo"][GameData.INITIAL_CITY] = "壊れている"
	var survived: GameSession = _restore_from(broken)
	_check(not survived.has_memo(GameData.INITIAL_CITY), "壊れたメモは捨てる", "残った")
