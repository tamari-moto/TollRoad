...
探索に使う装備は**5つのスロット**に挿して指定する。スロットに挿した装備だけが成功率に効き、失敗したときに失われる。**積荷に何個持っていても探索には一切関係しない。**
...
|---|---|---|
| 積載重量 | 食う | **食わない** |
| 市場で売る | できる | できない（先に積荷へ戻す） |
| 純資産への計上 | される | **される**（置き場所で純資産は変わらない） |
| 黒ゾーンの襲撃 | 全損する | **失われない** |
| 探索の成功率 | 影響しない | 1個につき+3% |
| 探索の失敗 | 影響しない | **全て失う** |

出し入れは日数を消費しない。積荷からスロットへ挿すと積荷から出るので積載が空き、スロットから戻すには積載に空きが要る（空きが無ければ戻せない）。

黒ゾーンの襲撃でスロットが無事なのは、襲撃が奪うのは「荷車に積んだ商品」であって身に着けた装備ではない、という区別による。装備を守る手段としてスロットを使えてしまうが、枠が5つしかないため抜け道としては小さい。
...
- スロットボーナスは、スロットに挿した戦闘装備（剣・弓・ローブ・革鎧・盾・戦鎚・外套・杖。交易用の装備をそのまま兼用する）1個につき+3%。同じ種類は3個までしか効かない（**種類を跨いで挿す方が伸びる**。5枠すべてを剣で埋めても+9%にしかならない）
- したがって到達できる上限は、5枠を別々の種類で埋めた **+15%**（成功率50%）
- 内部には合計+30%・成功率85%の上限が残っているが、スロット制では**どちらにも届かない**。枠数やボーナスを将来動かしたときの歯止めとして残してある
...
- 5日間、島の労働者の産出が2倍になる
- **スロットの中身は減らない。** 消費されるのは失敗したときだけ

**失敗時のリスク**

**スロットに挿した装備を全て失う**（最大5個）。積荷は資源も装備も無傷で、黒ゾーン襲撃の「積荷全損」とは別物。

賭けるものを明示的に選ばせる形にしてある。以前は積荷にある戦闘装備が自動的に対象となり、成功率に寄与する数（同種3個まで）と失う数が食い違っていた。装備は交易品でもあるため、市場間で装備を運んでいる最中に探索すると、賭けたつもりのない在庫まで巻き込まれていた。スロットに挿したものだけが対象になることで、**何を賭けているかが画面上で確定する**。
...
| 探索 | 現在地の成功率、5つの探索スロット（挿す/戻す）、討伐/遺跡探索ボタン | サイドパネル |
