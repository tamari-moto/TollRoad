...
- **既存コードとの乖離**: [economy_manager.gd](../scripts/autoload/economy_manager.gd) の `gold = 1000` は本仕様の 30,000 と一致しない。スキャフォールド時の仮値であり、実装時に修正が必要。
- **売却税の課税対象**: 5%が売上総額にかかるのか、利益にかかるのか。「売却額の5%」からは総額と読めるが明示されていない。
- **島倉庫の容量**: 上限があるか無限か。引き取りは「積載量の上限まで」とあるが、倉庫側の上限は未記載。
- **労働者の資源分布**: 「ランダムな資源を2個」の抽選が5資源で均等か、重み付きか。
- **価格のゆらぎの独立性**: 都市 × 品目ごとに独立して抽選するのか、品目単位の変動が全都市に共通で乗るのか。前者だと都市間の価格差が毎日大きく揺れる。
- **襲撃の判定単位**: 22%が移動1回あたりの判定か、経路の区間ごとの判定か。
- **破産時の扱い**: 所持シルバーが移動費に満たない場合の挙動。ゲームオーバーとするか、休息のみ可能として継続させるか。
- **60日終了時の積荷**: 純資産に基準価格 ×0.9 で算入されるが、これを強制換金として扱うか、単なる評価額とするか。
