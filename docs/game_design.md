...
積荷にある戦闘装備（剣・弓・ローブ・革鎧・盾・戦鎚・外套・杖）を、**成功率へ寄与した分だけ**失う。すなわち同じ種類につき3個まで（装備ボーナスの頭打ちと同じ数）で、4個目以降は残る。資源は無傷（黒ゾーン襲撃の「積荷全損」とは区別する）。

失う数を成功率の計算と同じ上限に揃えてあるのは、**賭けた分だけ失う**という対応を保つため。以前は種類ごとに全数を失っていたが、それだと剣を8個積んだ場合にボーナスは3個ぶん（+9%）しか付かないのに8個すべてを失い、成功率に一切寄与していない4個目以降まで没収されていた。装備は交易品でもあるため、市場間で装備を運んでいる最中の探索が、プレイヤーから見て理由の分からない重さで罰せられる状態だった。
