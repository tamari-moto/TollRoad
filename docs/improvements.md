...
### 1-b-2. 判明した既存の不安定: `scenario_m14.gd`（解消済み）

**移行とは無関係の既存の問題。** 一括ランナーを入れて初めて可視化された。
m14 の2件が実行のたびに結果を変えていた（同じ状態で5回流すと 3勝2敗）。
移行前のコードでも同じことを `git stash` で確認している。

**当初「ヘッドレスのフレーム進行が一定でないため」と診断したが、これは
誤りだった。** 真因は検査が `elapsed += 0.016` と**フレーム数に固定値を
掛けて経過時間を代用していた**こと。1フレームの実尺は環境と出力先で変わり、
実測では12フレームで93ms しか進まず、演出に必要な620ms に遠く届かない。
標準出力をファイルへ向けただけで結果が変わっていたのはこのためで、
「揺れ」ではなく**待ち方の誤り**だった。

修正は待ちを実時間で計る形へ（`Time.get_ticks_msec()`）。弧の標本も
飛翔の半ばまで進めてから採る。以後は安定して合格する。

教訓: **不安定に見えたら、揺れと決めつける前に原因を特定する。**
閾値を緩める方向で片付けていたら、検査は無意味になっていた。
...
**全段完了（2026-08-10）。** 実測 **18本 906件すべて合格**。

段9 で分かったこと: **`GameState._ready()` でセーブを消してはいけない。**
当初の設計どおり起動時のセッションを `start_new_game()` で作ると、その中の
`delete_save()` が走って「続きから」が永遠に出ない。

ただし**その回避策のまま「閉じたら save_game()」にしたのが誤りだった**。
起動時のセッションは1日目なので、進行中のセーブを1日目で塗り潰していた
（8日目のセーブが開始画面を閉じるだけで消えるのをレビューで指摘され、
再現して確認）。正しくは `delete_save()` してから作り直す。

レビューで見つかったセーブの寿命の不具合は3件（上書き、最終日のやり直し、
「続きから」失敗時の切断放置）。**いずれも「保存できるか」しか検査して
いなかったため素通りしていた。** いつ消えていつ残るかを見る検査を足した。
