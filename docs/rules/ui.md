...
**演出層（[fx_layer.gd](../../scripts/ui/fx_layer.gd)）はパネルの中に置かない。**
`Main.tscn` の `SidePanel` は `clip_contents = true`（開閉のスライドを綺麗に
見せるため）なので、市場パネルの内側に置いた飛翔物はパネルの縁で切られ、
HUD まで届かない。`UI/Root` の**最後の子**として吊ること
（`main.gd` の `_setup_trade_fx()`）。最後の子＝最前面。

**飛び先はパネルではなく main.gd が決める。** 市場パネルは
`traded(item_id, is_buy, origin)` で「何を・買ったか売ったか・どこから」だけを
知らせる。自分の外に何があるかを知らないパネルに飛び先を持たせると、
画面構成を変えるたびにパネル側を直すことになる。

飛び先は**その取引で増えるもの**へ向ける（買い＝HUD の積載、売り＝HUD の
シルバー）。数字を読まなくても、飛ぶ向きだけで何をした取引か分かる。
座標は HUD の `cargo_anchor()` / `silver_anchor()` が答える。
