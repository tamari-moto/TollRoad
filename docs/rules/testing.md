...
**大陸図（3D）だけを見たいときは確認用シーンを使う。** 本編を起動すると
HUD・タブ・日誌が地図の上に重なり、周回カメラでは寄れる距離にも限りがある。
`scenes/debug/MapPreview.tscn` は大陸図だけを FPS 操作で歩き回れる
（エディタなら F6。詳細は [status.md](../status.md)）:

```bash
"...Godot_v4.7.1-stable_win64.exe" --path . scenes/debug/MapPreview.tscn
```

ただしセッションを持たないので、**都市の選択・隊商・現在地の強調は本編でしか
確認できない**。絵作りはこちら、状態が絡む表示は本編、と使い分けること。
